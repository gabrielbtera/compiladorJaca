/* This file was generated by SableCC (http://www.sablecc.org/). */

package jaca.node;

import jaca.analysis.*;

@SuppressWarnings("nls")
public final class APinicializacaoPinicializacao extends PPinicializacao
{
    private TIdentificador _esq_;
    private TInicializacao _inicializacao_;
    private PExp _dir_;

    public APinicializacaoPinicializacao()
    {
        // Constructor
    }

    public APinicializacaoPinicializacao(
        @SuppressWarnings("hiding") TIdentificador _esq_,
        @SuppressWarnings("hiding") TInicializacao _inicializacao_,
        @SuppressWarnings("hiding") PExp _dir_)
    {
        // Constructor
        setEsq(_esq_);

        setInicializacao(_inicializacao_);

        setDir(_dir_);

    }

    @Override
    public Object clone()
    {
        return new APinicializacaoPinicializacao(
            cloneNode(this._esq_),
            cloneNode(this._inicializacao_),
            cloneNode(this._dir_));
    }

    @Override
    public void apply(Switch sw)
    {
        ((Analysis) sw).caseAPinicializacaoPinicializacao(this);
    }

    public TIdentificador getEsq()
    {
        return this._esq_;
    }

    public void setEsq(TIdentificador node)
    {
        if(this._esq_ != null)
        {
            this._esq_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._esq_ = node;
    }

    public TInicializacao getInicializacao()
    {
        return this._inicializacao_;
    }

    public void setInicializacao(TInicializacao node)
    {
        if(this._inicializacao_ != null)
        {
            this._inicializacao_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._inicializacao_ = node;
    }

    public PExp getDir()
    {
        return this._dir_;
    }

    public void setDir(PExp node)
    {
        if(this._dir_ != null)
        {
            this._dir_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._dir_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._esq_)
            + toString(this._inicializacao_)
            + toString(this._dir_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._esq_ == child)
        {
            this._esq_ = null;
            return;
        }

        if(this._inicializacao_ == child)
        {
            this._inicializacao_ = null;
            return;
        }

        if(this._dir_ == child)
        {
            this._dir_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._esq_ == oldChild)
        {
            setEsq((TIdentificador) newChild);
            return;
        }

        if(this._inicializacao_ == oldChild)
        {
            setInicializacao((TInicializacao) newChild);
            return;
        }

        if(this._dir_ == oldChild)
        {
            setDir((PExp) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}
